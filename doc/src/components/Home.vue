        ...
        <el-menu
          ...
          <template v-for="(item,index) in this.$router.options.routes" v-if="!item.hidden">
            <el-submenu :index="index+''" v-if="item.children.length>1" :key="index">
              ...
              <el-menu-item v-for="child in item.children" v-if="!child.hidden"
                            :index="item.path+'/'+child.path" :key="child.path"
                            :class="{'is-active':child.name===$router.currentRoute.name}">
            ...
            <template v-else>
              <el-menu-item :index="item.path+'/'+item.children[0].path"
                            :class="{'is-active':item.children[0].name===$router.currentRoute.name}">
